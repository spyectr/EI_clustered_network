...
function [J, params]=weightmatrix(paramsfile)
...
switch Network.clust
    case 'hom' % homogeneous clusters: all have the same size
        popsize=repmat(NcUnits,Q,1);
    case 'het' % heterogeneous clusters: different sizes
        Nc=[];
...
imagesc(J);
figset(gca,'neurons','neurons','weights',10);
colorbar;
subplot(2,1,2);
lambda=eig(J);
